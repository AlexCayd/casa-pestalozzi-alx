...
class ReservationClientSession
{
     ...
     */
    public static function crear(string $tipo, string $contactoNormalizado): void
    {
        self::start();
        ...
        $_SESSION[self::SESSION_KEY] = [
            'contact_type' => $tipo,
            'contact_normalized' => $contactoNormalizado,
            'verified_at' => $ahora,
            'expires_at' => $ahora + (ReservacionConfig::CLIENT_SESSION_MINUTES * 60),
        ];
    }
     ...
     *
     * @return array{contact_type: string, contact_normalized: string, verified_at: int, expires_at: int}|null
     */
    public static function obtener(bool $renovar = true): ?array
    {
        ...
        if (
            !in_array((string)($sesion['contact_type'] ?? ''), ContactoService::TIPOS, true)
            || trim((string)($sesion['contact_normalized'] ?? '')) === ''
        ) {
            unset($_SESSION[self::SESSION_KEY]);
            return null;
        }
        ...
        if ($renovar) {
            $_SESSION[self::SESSION_KEY]['expires_at'] =
                time() + (ReservacionConfig::CLIENT_SESSION_MINUTES * 60);
