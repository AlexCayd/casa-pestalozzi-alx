...
/* ---- Formulario ---- */
function initLegacyReservationForm() {
...
/**
 * Etapa 2: exploración -> retención/creación -> OTP cuando corresponde.
...
function initForm() {
  ...
  function showOtp(data, requestPayload) {
    ...
    startCountdown(data.verification_expires_at || data.otp_expires_at);
    otpInput.focus();
    if (window.ScrollTrigger) window.ScrollTrigger.refresh();
  }
