...
<form class="admin-menu__form" method="POST" action="<?php echo htmlspecialchars($action, ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <select id="rol" name="rol" required>
        <?php foreach ($roles as $rol) : ?>
            ...
            <option value="<?php echo htmlspecialchars($rol, ENT_QUOTES, 'UTF-8'); ?>" <?php echo $rolActual === $rol ? 'selected' : ''; ?>>
                <?php echo htmlspecialchars(ucfirst($rol), ENT_QUOTES, 'UTF-8'); ?>
            </option>
        <?php endforeach; ?>
    </select>
    ...
    <div class="admin-menu__form-actions">
        <button type="submit" class="admin-btn admin-btn--primary admin-menu__button admin-menu__button--primary">
