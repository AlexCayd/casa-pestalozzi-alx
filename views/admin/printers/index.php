<?php
/* El interruptor GLOBAL del servicio vive en Configuración > POS: es un ajuste
   del punto de venta y no una propiedad de ninguna estación. Aquí se lee para
   rotular las filas —una impresora activa con el servicio pausado NO imprime—,
   pero no se cambia. */
...
<section class="admin-menu admin-page">
    ...
    <?php /* El aviso sólo aparece con el servicio pausado: con todo funcionando
             no hay nada que decir y una barra fija encima de la tabla sería
             cromo. Enlaza a donde está el interruptor, que ya no es esta
             pantalla. */ ?>
    <?php if (!$servicioActivo) : ?>
        <section class="admin-panel admin-card admin-printers__paused" id="printers-service">
            <div>
                <h3>El servicio de impresión está pausado</h3>
                <p>No se envía nada a las impresoras. Los pedidos se guardan, descuentan inventario y llegan al tablero de producción como siempre. «Imprimir prueba» sigue funcionando: es con lo que se comprueba una estación antes de reanudar.</p>
            </div>
            <a class="admin-btn admin-btn--tinted admin-btn--tinted-ambar" href="/admin/configuracion/pos#pos-impresion-title">
                Reanudar en Configuración
            </a>
        </section>
    <?php endif; ?>
                        ...
                        <?php foreach ($impresoras as $impresora) : ?>
                            <tr data-row-href="/admin/printers/edit?id=<?php echo (int) $impresora->id; ?>">
                                ...
                                <?php /* Con el servicio pausado, una impresora activa
                                         ...
                                         servicio, y el aviso de arriba lo explica. */ ?>
                                <td>
